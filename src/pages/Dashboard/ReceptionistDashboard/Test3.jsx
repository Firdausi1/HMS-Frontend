...
const ReceptionistProfile = () => {
    ...
    if (isEditable) {
      try {
        const response = await axios.put(
          `http://localhost:3001/api/receptionist/${receptionist.id}`,
          formData
        );
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:3001/api/receptionist/change-password/${receptionist.id}`,
        {
    ...
    <div className="min-h-screen bg-blue-50 flex flex-col items-center py-6">
      <div className="flex gap-[40px] items-center">
        ...
        <form onSubmit={handleSubmitProfile} className="bg-blue-200 rounded-lg shadow-md p-6 w-[500px] max-w-lg mb-8">
          ...
          >
            {isEditable ? "Save Changes" : "Edit Information"}
          </button>
        </form>
      </div>
