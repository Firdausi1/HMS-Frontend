...
function DoctorDashboard() {
  const { logout, user } = useContext(AuthContext); // Access the doctor's first name and setDoctor function from context
  ...
  const handleViewAppointments = async () => {
    try {
      const response = await getRequest("appointment");
      if (!response.ok) {
    ...
    <div className="min-h-screen bg-gray-100 p-6 relative">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">
          Welcome, Dr. {user ? user?.firstName : "Name"}
        </h1>
        <button
