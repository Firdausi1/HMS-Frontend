...
function DoctorLogin() {
    ...
    const { setDoctor } = useUser(); // Get the setDoctor function from context

    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('http://localhost:3001/api/doctor/login', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                console.log('Doctor first name set to:', result.data.firstName); // Access firstName from data
                setDoctor(result.data.firstName); // Set the first name in context
                
                // Store doctor's information in local storage
        ...
        <div className="flex items-center justify-center h-screen bg-gradient-to-r from-blue-500 to-blue-300">
            <form className="bg-white p-10 rounded-lg shadow-lg w-96" onSubmit={handleLogin}>
                ...
                <p className="mt-4 text-center text-gray-600">
                    Don't have an account? <Link to="/DoctorRegister" className="text-blue-500 hover:underline">Sign up</Link>
                </p>
            </form>
        </div>
    );
